...
class GenPlate():
    def __init__(self):
        self.font = ImageFont.truetype("RoadNumbers2.0.ttf", size=500)
        self.plate = 0
        self.PADDING_H = 36
        self.H_WORD = 58
        self.H_NUM = 76
        self.PADDING_LEFT = 38#41
        self.t = ""
        self.full_text = ""
        self.LETTER = "ABEKMHOPCTYX"
        self.DIGIT = "0123456789"
        self.all_speasing = 0
    def gen(self, h_word, pad_top, speasing):  #h_word, pad_top, speasing
        ...
        M, out_of_bounds = make_affine_transform(
                            ...
                            scale_variation=1.0,
                            translation_variation=1.0)
        plate = cv2.warpAffine(np.array(img), M, (shape[1], shape[0]))
                        
        plate_mask_t = np.zeros(np.array(img).shape) 
        plate_mask_t[:,:,:] = 255
        #plate = cv2.warpAffine(plate, M, (shape[1], shape[0]))
        plate_mask = cv2.warpAffine(plate_mask_t, M, (shape[1], shape[0]))
        img = np.array(img)
        #imgs(plate_mask_t)  
        for iw in range(plate_mask.shape[0]):
            for ih in range(plate_mask.shape[1]):
                for ic in range(plate_mask.shape[2]):
                    if plate_mask[iw,ih,ic] == 255:
                        plate_mask_t[iw,ih,ic] = plate[iw,ih,ic]  #255#
                        #plate_mask_t[iw,ih,ic] = 255#plate[iw,ih,ic] 
        img = cv2.resize(np.array(plate_mask_t),dim)
        
        #imgs(img)
        self.plate[pad_top:(pad_top+h_word), (self.PADDING_LEFT-x_x):((self.PADDING_LEFT-x_x)+int(w*kef))] = img[:,:,0]#255#
        self.PADDING_LEFT += (img.shape[1]-self.font.getmask(self.t).getbbox()[0]//2)
        self.PADDING_LEFT += speasing
        
    def gen2(self, h_word, pad_top, speasing):
        #temp = self.t
        
        img = COORD.file[self.t.lower()][-1]
        shape = np.array(img).shape
        h = shape[0]
        w = shape[1]
        kef = h_word/h
        #print (shape, h_word, pad_top, speasing, (int(w*kef),h_word))
        dim = (int(w*kef),h_word)
#        
        img = cv2.resize(np.array(img),dim)
        #imgs(img)
        self.plate[pad_top:pad_top+h_word, self.PADDING_LEFT:(self.PADDING_LEFT)+int(w*kef)] = img[:,:,0]
        self.PADDING_LEFT += img.shape[1]
        self.PADDING_LEFT += speasing
#        print (dim, self.t, x_x)
                
    def clear_plate(self):
        plate = np.zeros((112,520)) #np.ones
    ...
    def gen_text_plate2(self):
        ...
        self.full_text += self.t
        self.gen2(test.H_NUM, test.PADDING_H//2, 21)

        self.t = random.choice(self.LETTER)#"m"
        self.full_text += self.t
        self.gen2(test.H_WORD, test.PADDING_H, 11)
...
#Получить координаты
COORD = DATA()
COORD.parseIMG("car_number")   
#print (len(COORD.file))
#P_1_char = []
#P_1_int = []
#for o in COORD.file:
#    if o in test.DIGIT:
