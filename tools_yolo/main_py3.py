...
class LabelTool():
    def __init__(self, master):
        ...
        self.cla_can_temp = []
        self.classcandidate_filename = 'class.txt'#'coco.names'
    ...
    def loadDir(self):
        ...
        if not os.path.isdir(self.imageDir):
            ...
            return
        f = DATA()
        f.parseIMG(self.imageDir)
        for igh, gh in enumerate(f.file):
                 self.imageList.append(f.file[gh][0])


        #f = glob.glob("data/bicycles4x4/img/*")
        #f = glob.glob("Labels/*")
        #g = f.readlines()
#        f = DATA()
#        f.parseIMG("srplate")
...
#                 self.imageList.append(f.file[gh][0])
    ...
    def loadImage(self):
        ...
        #self.factor = max(416/1000, 416/1000., 1.)
        #self.img = self.img.resize((int(416/self.factor), int(416/self.factor)))
    ...
    def saveImage(self):
        ...
        with open(self.labelfilename, 'w') as f:
            ...
            for bbox in self.bboxList:
                ...
                #                                  int(int(bbox[3])*self.factor), "Bike"))#bbox[4]))
                f.write("{} {} {} {} {}\n".format(bbox[0],
                                                  int(int(bbox[1])*self.factor),
                                                  int(int(bbox[2])*self.factor),
                                                  int(int(bbox[3])*self.factor),
                                                  int(int(bbox[4])*self.factor)))#bbox[4]))
